...
namespace gta_base::memory::scanner {
  ...
  Handle Range::scan(Pattern const &sig)
  {
    ...
    if (auto result = scan_pattern(data, length, base_, size_); result) {
      return result;
    }
  ...
  Handle Range::bruteforce_scan(const Pattern& sig) {
    auto data = sig.bytes_.data();
    auto length = sig.bytes_.size();

    const auto scan_end = size_ - length;
    for (std::uintptr_t i = 0; i < scan_end; ++i) {
      if (pattern_matches(base_.add(i).as<std::uint8_t*>(), data, length)) {
        return base_.add(i);
      }
    }
  ...
  Handle Range::bruteforce_scan(char* sig) {
    return bruteforce_scan(Pattern(sig));
  }
}