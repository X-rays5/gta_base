...
#ifndef GTABASE_SIGSCANNER_HPP
#define GTABASE_SIGSCANNER_HPP
#include <string>
...
namespace gta_base {
  namespace memory {
    ...
    public:
       ...
       */
      constexpr explicit MemoryHandle(void* ptr = nullptr) : ptr_(ptr)
      {
      }
       ...
       */
      explicit MemoryHandle(std::uintptr_t ptr) : ptr_(reinterpret_cast<void*>(ptr))
      {
      }
    ...
    public:
       ...
       */
      constexpr explicit MemoryRegion(MemoryHandle base, std::size_t size) : base_(base), size_(size)
      {
      }
    ...
    public:
       ...
       */
      explicit Module(HMODULE hmod) : MemoryRegion(MemoryHandle(hmod), MemoryHandle::GetModuleSize(hmod)) {
    ...
    public:
      ...
      MemoryHandle Scan(const std::string& name, MemoryRegion region = Module(nullptr)) {
        ...
        if (result.Rip().As<void*>() == nullptr) {
          LOG_WARN("Failed to find {}", name);
        } else {
          #ifndef NDEBUG
            LOG_INFO("Found {} at 0x{}", name, result.Rip().As<void*>());
          #else
            LOG_INFO("Found {}", name);
          #endif
        }
