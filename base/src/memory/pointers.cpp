...
namespace gta_base::memory {
    Pointers::Pointers() {
      ...
      main_batch.add(VAR_NAME(model_spawn_bypass_), xorstr_("48 8B C8 FF 52 30 84 C0 74 05 48"), [this](scanner::Handle ptr){
        model_spawn_bypass_ = ptr.add(8).as<decltype(model_spawn_bypass_)>();
      });
