  ...
  void SelectedPlayerTab() {
    ...
    kMANAGER->AddSubmenu(Submenus::PlayerListSelectedPlayer, "", [](Submenu* sub) {
      ...
      sub->AddOption(option::ExecuteOption("option/teleport_to_selected", "", [] {
        fiber::kPOOL->AddJob([] {
          rage::util::LoadGroundAtCoord(*kPLAYER_MGR->GetSelectedPlayer()->Ped()->m_navigation->get_position());
          rage::util::Teleport((rage::fvector3) *kPLAYER_MGR->GetSelectedPlayer()->Ped()->m_navigation->get_position(), true);
        });
      }));
