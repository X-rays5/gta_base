  ...
  public:
    ...
    inline void SetActiveSubmenu(Submenus id) {
      ...
      if (entry != submenus_.end()) {
        submenus_stack_.top()->Clear();
        submenus_stack_.push(entry->second);
        submenus_stack_.top()->CreateOptions();
        UpdateCurOptHintTxt();
      }
    }
    ...
    inline std::shared_ptr<Submenu> GetCurSubmenu() {
      std::unique_lock lock(mtx_);
      if (!submenus_stack_.empty())
    ...
    FORCE_INLINE bool IsOptionHotkeyAble(const std::string& name_key) {
      std::unique_lock lock(mtx_);
      for (auto&& sub : submenus_) {
        auto& sub_menu = sub.second;

        if (!sub_menu->GetOptionCount())
          sub_menu->CreateOptions();

        for (std::size_t i = 0; i < sub_menu->GetOptionCount(); i++) {
          auto opt = sub_menu->GetOption(i);
          if (name_key == opt->GetNameKey() && opt->HasFlag(OptionFlag::kHotkeyable)) {
            sub_menu->Clear();
            return true;
          }
        }
    ...
    FORCE_INLINE std::shared_ptr<d3d::draw::DrawList> GetDrawList() const {
      return draw_list_;
    }

    FORCE_INLINE std::recursive_mutex* GetMutex() {
      return &mtx_;
    }
