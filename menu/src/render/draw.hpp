...
namespace base::render {
  class DrawQueue {
    public:
      ...
      FORCE_INLINE void AddCommand(T command) { draw_commands_.push_back(std::make_unique<T>(command)); }

      void Draw() { for (auto&& command : draw_commands_) { command->Draw(); } }

      FORCE_INLINE void Clear() { draw_commands_.clear(); }
  ...
  class DrawQueueBuffer {
    public:
      ...
      inline void AddCommand(Args&&... command) { draw_queue_[write_idx_].AddCommand(std::forward<Args>(command)...); }
