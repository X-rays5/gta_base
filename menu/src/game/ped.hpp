...
namespace base::menu::game {
  ...
  public:
    ...
    [[nodiscard]] Vehicle GetVehicle(bool includeEntering = false) const;
    ...
    void IntoVehicle(const Vehicle& vehicle, int seat) const;
    /// The same, picking a free seat - false when the vehicle has none.
    bool IntoVehicle(const Vehicle& vehicle) const;
    ...
    void Teleport(const rage::Vector3& position, bool withVehicle = false) const;
  };
